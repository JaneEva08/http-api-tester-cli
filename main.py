"""
Тестер HTTP-запросов — CLI-инструмент для тестирования GET-запросов к API.
...
def do_get_by_country() -> None:
    """
    Получить JSON по стране и выделить из него ключевые поля
    (название, столица, население, валюта).

    restcountries.com/v3.1 на момент написания кода уже отключён и требует
    платной регистрации, поэтому здесь используется другой бесплатный
    источник без ключа — sampleapis.com, который отдаёт список всех стран
    одним GET-запросом.
    """
